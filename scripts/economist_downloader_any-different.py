...
Formats disponibles :
    ...
    a4landscape: 🖨️  A4 Premium Paysage trois colonnes (297×210mm)
    all        : ✨ Tous les formats (6 PDFs)
...
def build_css(fmt: dict = None) -> str:
    ...
    # A4 premium: multi-column article body
    column_css = ""
    if two_col:
...
class EconomistDownloader:
    def __init__(self, output_dir=None, img_width='600', include_images=True,
                 delay=1.0, edition_date=None, fmt=None):
...
{full_html}
    ...
    def run(self, list_only=False, generate_all=False):
...
Exemples:
  ...
  %(prog)s --width 960 --delay 0.5  # Images HD, téléchargement rapide
    ...
    parser.add_argument('--format', '-f',
                        choices=['phone', 'ereader', 'tablet7', 'tablet10', 'a4premium', 'a4landscape', 'all'],
                        default=None,
