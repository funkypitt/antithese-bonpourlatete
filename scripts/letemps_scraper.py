...
def _download_image_bytes_letemps(url: str, session=None) -> tuple[bytes, str] | None:
...
def generate_premium_landscape_pdf(
    ...
    """Generate a premium A4 landscape PDF optimised for comfortable reading.

    Same design language as the portrait premium (New Yorker-inspired),
    but laid out on landscape A4 (297 × 210 mm) with three columns and
    slightly larger body text for a pleasant, easy-to-read experience.
    """
    from weasyprint import HTML
...
<style>
    /* ================================================================
       ...
       Mise en page paysage, 3 colonnes, typographie confortable
       ================================================================ */
    ...
    * {{
    ...
    /* ════════════════════════════════════════════════════════════════
       TABLE OF CONTENTS  (3 columns in landscape)
       ════════════════════════════════════════════════════════════════ */
    .toc-page {{
        page-break-after: always;
        padding-top: 4mm;
    ...
    /* ════════════════════════════════════════════════════════════════
       ARTICLES  (3-column body for comfortable landscape reading)
       ════════════════════════════════════════════════════════════════ */
    .pm-article {{
        page-break-before: always;
    }}
    ...
    /* Three-column body — wider landscape page */
    .pm-body {{
        column-count: 3;
        column-gap: 7mm;
        column-rule: 0.3pt solid #e8e8e8;
...
def main():
    ...
    # --- Download logo (after session, to avoid pre-login requests to letemps.ch) ---
    ...
    for fmt_key in formats_to_gen:
        if fmt_key == "epub":
            date_iso = td.isoformat()
            # Flatten dict[section, list[Article]] → list[dict]
            epub_articles = []
            for section, arts in all_arts.items():
                for art in arts:
                    epub_articles.append({
                        ...
                        "category": section,
                        "lead": art.lead,
                        "content_html": art.body,
                        "image_url": art.image_url,
                        "image_data_uri": image_cache.get(art.image_url, ""),
                        "image_caption": art.image_caption,
                    })
            op = out_dir / f"{date_iso}-letemps.epub"
            generate_epub(
                epub_articles,
                publication_title="Le Temps",
                edition_title=f"Édition du {td:%d.%m.%Y}",
                date_str=date_iso,
                output_path=op,
                publisher="Le Temps",
                image_fetcher=lambda url: _download_image_bytes_letemps(url, img_session),
            )
            generated.append(("EPUB", op))
            continue
